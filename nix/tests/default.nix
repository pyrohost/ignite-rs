...
let
  ...
  mkTest = { name ? "", user ? "root", flakes ? true, isLocal ? true, deployArgs }: let
    nodes = {
      server = { nodes, ... }: {
        ...
        virtualisation.additionalPaths = lib.optionals (!isLocal) [
          ...
          pkgs.ignite-rs.ignite-rs
        ];
      };
      client = { nodes, ... }: {
        imports = [ (import ./common.nix { inherit inputs pkgs flakes; }) ];
        environment.systemPackages = [ pkgs.ignite-rs.ignite-rs ];
    ...
    flakeInputs = ''
      ignite-rs.url = "${../..}";
      ignite-rs.inputs.utils.follows = "utils";
      ignite-rs.inputs.flake-compat.follows = "flake-compat";
